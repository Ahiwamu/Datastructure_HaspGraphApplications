public class HashTable {
    ...
    public void addString(String s){
        ...
        int index = getIndex(s);    //get index
        arr[index] = s; //set arr[index] is string s
    }
    ...
    public int getIndex(String s){
        int index = polyHash(s, p, x);   //get index
        for(int k = 0; k < capacity; k++){
            index = (index + k) % capacity;    
            if(arr[index] == null || arr[index].equals(s)){  //check arr[index] is empty or equals string
                return index;
            }
        }
        for(int k = 0; k < capacity; k++ ){ //find index that empty
            if(arr[k] == null){
                return k;
            }
        }
    ...
    public static int polyHash(String s, int p, int x){
        int hash = 0;
        int power = 1;
        for(int i = 0; i < s.length(); i++){    //go to all charactor in String s for hashing
            char a = s.charAt(i);   
            hash += ((power % p) * (int)a) % p;
            power = (power * x) % p;
        }
        hash = hash % p;
        return hash;
